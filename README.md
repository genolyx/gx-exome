...
Nextflow 기반 GX-Exome 분석 파이프라인입니다. WES(Whole Exome Sequencing) FASTQ 입력으로부터 변이 호출, CNV/SV 검출, 반복 확장 분석, 유사유전자 해석까지 수행하며, **service-daemon**과 연동하여 annotation, review, report 생성을 지원합니다.
...
```bash
# 1. Clone repository
git clone https://github.com/genolyx/gx-exome.git
cd gx-exome
...
│   ├── docker-compose.yml # gx-exome 컨테이너만
